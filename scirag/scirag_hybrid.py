"""
    ...
    Classes/Functions:
        - SciRagHybrid (line 126):
            - _setup_cost_tracking() (line 229)
            - _count_tokens(text: str) -> int (line 253)
            - _calculate_generation_cost(input_tokens: int, output_tokens: int, thinking_enabled: bool = False) -> float (line 262)
            - _log_cost_summary(generation_cost: float, input_tokens: int, output_tokens: int) (line 280)
            - _get_collection_name() (line 306)
            - get_total_cost() -> float (line 310)
            - get_cost_breakdown() -> dict (line 313)
            - store_to_chromadb() (line 333)
            - load_chromadb_collection() (line 386)
            - _initialize_embeddings_and_vector_db() (line 426)
            - _create_vector_db_after_embeddings() (line 448)
            - query_chromadb(query, n_results=5) (line 482)
            - _count_tokens(text: str) -> int (line 497)
            - _get_texts() (line 501)
            - _build_tf_idf_index() (line 535)
            - _embed_texts_gemini() -> np.ndarray (line 539)
            - _embed_texts_openai() -> np.ndarray (line 584)
            - _embed_texts() -> np.ndarray (line 624)
            - _embed_query_gemini(query: str) -> np.ndarray (line 639)
            - _embed_query_openai(query: str) -> np.ndarray (line 682)
            - _embed_query(query: str) -> np.ndarray (line 701)
            - get_embeddings() (line 708)
            - load_embeddings() (line 711)
            - _create_faiss_index(use_gpu: bool = False) -> faiss.Index (line 718)
            - _save_index() (line 746)
            - semantic_search(query: str, n_results: int = 20) -> Dict[str, Any] (line 782)
            - lexical_search(query: str, n_results: int = 20) -> List[Dict] (line 802)
            - hybrid_search(query: str, n_results: int = TOP_K, semantic_weight: float = semantic_weight) -> List[Dict] (line 827)
            - get_chunks(query: str) (line 863)
            - delete_vector_db() (line 866)
            - get_response(query: str) (line 869)
            - format_agent_output(response) (line 899)
