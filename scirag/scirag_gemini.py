"""
    ...
    Classes/Functions:
        - Paper (line 34):
        - GeminiGroundedAgent (line 45):
            - _count_tokens(text: str) -> int (line 147)
            - _calculate_cost(input_tokens: int, output_tokens: int) -> float (line 156)
            - _log_usage_and_cost(input_tokens: int, output_tokens: int, total_cost: float) (line 165)
            - get_total_cost() -> float (line 187)
            - _check_rate_limit(text: str) (line 190)
            - _initialize_gemini() (line 211)
            - _create_gemini_prompt() -> str (line 260)
            - get_response(query: str) -> str (line 293)
            - parse_structured_response(json_response: str) -> str (line 359)
            - _format_sources(source_info: List[str]) -> str (line 390)
            - _format_fallback_response(response_text: str) -> str (line 411)
